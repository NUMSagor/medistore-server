...
const orderController = {
    ...
    getMyOrders: async (req: Request, res: Response) => {
        try {
            if (!req.user) return res.status(401).json({ error: "Unauthorized" });
            const orders = await orderService.getByCustomer(req.user.id);
            res.status(200).json(orders);
        } catch (err) {
            res.status(400).json({ error: (err as Error).message });
        }
    ...
    getSellerOrders: async (req: Request, res: Response) => {
        try {
            if (!req.user) return res.status(401).json({ error: "Unauthorized" });
            const orders = await orderService.getBySeller(req.user.id);
            res.status(200).json(orders);
        } catch (err) {
            res.status(400).json({ error: (err as Error).message });
        }
    ...
    updateOrderStatus: async (req: Request, res: Response) => {
        try {
            ...
            const order = await orderService.updateStatus(orderId, status as OrderStatus, sellerId);
            res.status(200).json(order);
        } catch (err) {
            res.status(400).json({ error: (err as Error).message });
        }
